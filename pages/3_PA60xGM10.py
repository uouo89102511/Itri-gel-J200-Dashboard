...
<style>
    ...
    .section-title { font-size: 0.85rem; font-weight: 600; color: #42a5f5; text-transform: uppercase; letter-spacing: 0.1em; margin: 16px 0 8px 0; padding-bottom: 4px; border-bottom: 2px solid #1e4976; }
    .kpi-card { background: linear-gradient(135deg, #0d2137 0%, #102840 100%); border: 1px solid #1e4976; border-radius: 10px; padding: 12px 14px; text-align: center; height: 100%; }
    ...
    .upload-box { background: rgba(13,33,55,0.6); border: 1px dashed #1e4976; border-radius: 10px; padding: 16px; margin-bottom: 12px; }
    .file-chip { display: inline-block; background: rgba(102,187,106,0.1); border: 1px solid rgba(102,187,106,0.3); border-radius: 16px; padding: 2px 9px; font-size: 0.72rem; color: #66bb6a; margin: 1px; }
    .highlight-box { background: linear-gradient(90deg, rgba(13,33,55,0.5), rgba(21,101,192,0.2), rgba(13,33,55,0.5)); border: 1px solid #4fc3f7; border-radius: 10px; padding: 14px 22px; text-align: center; margin: 10px 0; }
    .highlight-box-orange { background: linear-gradient(90deg, rgba(13,33,55,0.5), rgba(255,138,101,0.18), rgba(13,33,55,0.5)); border: 1px solid #ff8a65; border-radius: 10px; padding: 14px 22px; text-align: center; margin: 10px 0; }
</style>
...
def load_pa60_single(file_bytes):
    ...
    verify_components = ["L2_kW_total (壓縮機總電)","L3_kW_total (除霜電熱)","L4_除霧電熱","L4_冷凝風扇","L4_蒸發風扇"]
...
def add_gm10_derived(df):
    ...
    outside_temp_keys = [s['col_key'] for s in OUTSIDE_SENSORS.values() if s['col_key'] in df.columns and s.get('unit') == '°C']
...
with st.sidebar:
    ...
    pa60_files = st.file_uploader("⚡ PA60 電力 CSV（多選）", type=["csv"], accept_multiple_files=True, key="pa60_uploads")
    gm10_files = st.file_uploader("❄️ GM10 溫度 CSV（多選）", type=["csv"], accept_multiple_files=True, key="gm10_uploads")
    st.markdown("---")
    st.markdown("### ⚙️ 重採樣")
...
if not pa60_files or not gm10_files:
    ...
    st.info("👈 請在左側同時上傳 **PA60** 與 **GM10** CSV（皆支援多選整月）。")
    st.stop()
...
with st.spinner("載入並處理資料中..."):
    ...
    total = len(pa60_files) + len(gm10_files); cnt = 0
    for f in pa60_files:
        try: pa60_dfs.append(load_pa60_single(f.read()))
        except Exception as e: pa60_errs.append((f.name, str(e)))
        cnt += 1; progress.progress(cnt/total, text=f"PA60 {cnt}/{len(pa60_files)}: {f.name}")
    for f in gm10_files:
        try: gm10_dfs.append(load_gm10_single(f.read()))
        except Exception as e: gm10_errs.append((f.name, str(e)))
        cnt += 1; progress.progress(cnt/total, text=f"GM10: {f.name}")
    progress.empty()
...
if not pa60_dfs or not gm10_dfs:
    st.error("PA60 或 GM10 沒有成功載入"); st.stop()
...
# 動態建議重採樣
overlap_days_raw = max(0, (min(df_pa60['時間'].max(), df_gm10['時間'].max()) -
                            max(df_pa60['時間'].min(), df_gm10['時間'].min())).total_seconds() / 86400)
if overlap_days_raw > 14:
    default_idx = 5  # 每日
elif overlap_days_raw > 3:
    default_idx = 3  # 15 分鐘
else:
    default_idx = 2  # 5 分鐘

resample_options = ["原始","1 分鐘","5 分鐘","15 分鐘","30 分鐘","1 小時","每日平均"]
resample_map = {"原始":None,"1 分鐘":"1min","5 分鐘":"5min","15 分鐘":"15min","30 分鐘":"30min","1 小時":"1h","每日平均":"1D"}

with st.sidebar:
    resample_opt = st.selectbox(
        f"資料頻率（重疊 {overlap_days_raw:.1f} 天，建議 {resample_options[default_idx]}）",
        resample_options, index=default_idx
    )
    rs_rule = resample_map[resample_opt]
    is_daily_mode = (rs_rule == "1D")

    st.markdown("---")
    st.markdown("### 📋 電力欄位")
    selected_pa60_cols = st.multiselect("PA60 電力欄位", list(PA60_POWER_COLS.keys()),
        default=["L1 冷庫總電 (kW)", "驗算總電 (kW)"], key="pa60_cols")
    ...
    selected_gm10_cols = st.multiselect("GM10 溫度欄位", list(gm10_temp_options.keys()),
        default=["庫內 8 點總平均", "庫外 5 點總平均", "CH101 壓縮機"], key="gm10_cols")
...
overlap_start = max(pa60_ts, gm10_ts); overlap_end = min(pa60_te, gm10_te)
...
c2.markdown(f"""<div class='kpi-card'>
    <div class='kpi-label'>⚡ PA60 資料筆數</div>
    <div class='kpi-value' style='font-size:1.2rem; color:#f5a623;'>{len(df_pa60):,}</div>
    <div class='kpi-unit'>重採樣後·{resample_opt}</div>
</div>""", unsafe_allow_html=True)
...
c4.markdown(f"""<div class='kpi-card'>
    <div class='kpi-label'>❄️ GM10 資料筆數</div>
...
if has_overlap:
    ...
    st.error("⚠️ PA60 與 GM10 資料時間範圍**無重疊**"); st.stop()

# Time slider
st.markdown("<div class='section-title'>⏱️ 選擇分析時間範圍</div>", unsafe_allow_html=True)
time_range = st.slider("時間範圍",
    min_value=overlap_start.to_pydatetime(), max_value=overlap_end.to_pydatetime(),
    value=(overlap_start.to_pydatetime(), overlap_end.to_pydatetime()),
    format="MM/DD HH:mm" if not is_daily_mode else "YYYY-MM-DD",
    label_visibility="collapsed")
t0 = pd.Timestamp(time_range[0]); t1 = pd.Timestamp(time_range[1])
...
if len(dff_pa60) == 0 or len(dff_gm10) == 0:
    st.error("選取範圍內無足夠資料。"); st.stop()

# Daily mode markers helper
line_mode = 'lines+markers' if is_daily_mode else 'lines'
marker_size = 9 if is_daily_mode else 0
...
l1_col = "L1_kW_total (冷庫總電)"; ver_col = "驗算總電 (kW)"
if l1_col in dff_pa60.columns and ver_col in dff_pa60.columns:
    s_l1  = pd.to_numeric(dff_pa60[l1_col],  errors='coerce')
    s_ver = pd.to_numeric(dff_pa60[ver_col], errors='coerce')
    t_hr = pd.DatetimeIndex(dff_pa60['時間']).astype(np.int64).values / 1e9 / 3600
    l1_v  = s_l1.fillna(0).values; ver_v = s_ver.fillna(0).values
    dt = np.diff(t_hr)
    l1_kwh  = float(np.sum((l1_v[:-1]  + l1_v[1:])  / 2 * dt))
    ver_kwh = float(np.sum((ver_v[:-1] + ver_v[1:]) / 2 * dt))
    ...
    fig_pwr = go.Figure()
    fig_pwr.add_trace(go.Scatter(x=dff_pa60['時間'], y=s_l1, name='L1 冷庫總電',
        mode=line_mode, line=dict(width=2.2, color='#4fc3f7'),
        marker=dict(size=marker_size),
        hovertemplate="L1: %{y:.3f} kW<extra></extra>"))
    fig_pwr.add_trace(go.Scatter(x=dff_pa60['時間'], y=s_ver, name='驗算總電',
        mode=line_mode, line=dict(width=2.2, color='#66bb6a', dash='dot'),
        marker=dict(size=marker_size),
        hovertemplate="驗算: %{y:.3f} kW<extra></extra>"))
    fig_pwr.add_trace(go.Scatter(x=dff_pa60['時間'], y=(s_l1 - s_ver), name='差值 (L1 - 驗算)',
        mode=line_mode, line=dict(width=1, color='#ffd54f', dash='dash'),
        marker=dict(size=marker_size),
        hovertemplate="差: %{y:.3f} kW<extra></extra>", visible='legendonly'))
    ...
    if is_daily_mode:
        fig_pwr.update_xaxes(tickformat='%m/%d')
    st.plotly_chart(fig_pwr, use_container_width=True)
else:
    st.warning("⚠️ PA60 資料缺少 L1 冷庫總電或驗算總電所需欄位")

# Temperature Summary
st.markdown("<div class='section-title'>🌡️ 庫內外溫度統計（時間範圍內平均）</div>", unsafe_allow_html=True)
st.markdown("**❄️ 庫內 8 點 (CH1 ~ CH8)**")
in_row1 = st.columns(4); in_row2 = st.columns(4)
...
out_row = st.columns(6); out_items = list(OUTSIDE_SENSORS.items())
for i, (name, s) in enumerate(out_items):
    if s['col_key'] not in dff_gm10.columns: continue
    sval = pd.to_numeric(dff_gm10[s['col_key']], errors='coerce').dropna()
    if len(sval) == 0: continue
    avg_v = sval.mean(); unit = s['unit']
    color = "#80deea" if unit == '%RH' else ("#ff8a65" if name == 'CH101' else "#a5d6a7")
...
if valid_pa60 or valid_gm10:
    ...
    if show_overlap:
        ...
        for i, (label, col) in enumerate(valid_pa60):
            ...
            fig.add_trace(go.Scatter(x=dff_pa60['時間'], y=s, name=f"⚡ {label}",
                mode=line_mode, line=dict(width=2, color=POWER_COLORS[i % len(POWER_COLORS)]),
                marker=dict(size=marker_size),
                hovertemplate=f"⚡ {label}: %{{y:.3f}} kW<extra></extra>"), secondary_y=False)
        for i, (label, col) in enumerate(valid_gm10):
            s = pd.to_numeric(dff_gm10[col], errors='coerce')
            fig.add_trace(go.Scatter(x=dff_gm10['時間'], y=s, name=f"🌡️ {label}",
                mode=line_mode, line=dict(width=2, color=TEMP_COLORS[i % len(TEMP_COLORS)], dash='dot'),
                marker=dict(size=marker_size),
                hovertemplate=f"🌡️ {label}: %{{y:.2f}}°C<extra></extra>"), secondary_y=True)
            ...
            if valid_pa60:
                for i, (label, col) in enumerate(valid_pa60):
                    ...
                    fig.add_trace(go.Scatter(x=dff_pa60['時間'], y=s, name=f"⚡ {label}",
                        mode=line_mode, line=dict(width=2, color=POWER_COLORS[i % len(POWER_COLORS)]),
                        marker=dict(size=marker_size),
                        hovertemplate=f"⚡ {label}: %{{y:.3f}} kW<extra></extra>"), row=row_idx, col=1)
                row_idx += 1
            if valid_gm10:
                for i, (label, col) in enumerate(valid_gm10):
                    s = pd.to_numeric(dff_gm10[col], errors='coerce')
                    fig.add_trace(go.Scatter(x=dff_gm10['時間'], y=s, name=f"🌡️ {label}",
                        mode=line_mode, line=dict(width=2, color=TEMP_COLORS[i % len(TEMP_COLORS)]),
                        marker=dict(size=marker_size),
                        hovertemplate=f"🌡️ {label}: %{{y:.2f}}°C<extra></extra>"), row=row_idx, col=1)
            fig.update_layout(
                plot_bgcolor='rgba(10,20,40,0.9)', paper_bgcolor='rgba(0,0,0,0)',
                height=300 * n_rows, hovermode='x unified', dragmode='zoom',
...
if show_corr and valid_pa60 and valid_gm10:
    st.markdown("<div class='section-title'>🔗 相關係數分析</div>", unsafe_allow_html=True)
    pa60_sel = dff_pa60[['時間'] + [c for _, c in valid_pa60]].set_index('時間')
    gm10_sel = dff_gm10[['時間'] + [c for _, c in valid_gm10]].set_index('時間')
    align_rule = '1D' if is_daily_mode else '1min'
    pa60_a = pa60_sel.resample(align_rule).mean()
    gm10_a = gm10_sel.resample(align_rule).mean()
    merged = pd.concat([pa60_a, gm10_a], axis=1).dropna()
    if len(merged) > 5:
        ...
        fig_corr = go.Figure(go.Heatmap(
            ...
            colorscale=[[0,'#1a237e'],[0.5,'#0d2137'],[1,'#4fc3f7']],
            zmid=0, zmin=-1, zmax=1,
            text=np.round(cross_corr.values, 2), texttemplate="%{text}",
        ...
        st.caption(f"r ≈ +1 正相關，r ≈ -1 負相關（以 {align_rule} 為基準對齊）")
            ...
            for label, col in valid_gm10:
                ...
                if len(s):
                    rows.append({"欄位": label, "平均 (°C)": f"{s.mean():.2f}",
                                 ...
                                 "標準差": f"{s.std():.3f}"})
            if rows: st.dataframe(rows, hide_index=True, use_container_width=True)
        else: st.info("未選擇 GM10 欄位")

st.markdown("---")
st.markdown("""<div style='text-align:center; color:#3a5a7a; font-size:0.78rem;'>
