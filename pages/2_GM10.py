...
<style>
    ...
    .page-header { background: linear-gradient(90deg, #071525, #0a1e32); border: 1px solid #0e3a5c; border-left: 4px solid #4fc3f7; border-radius: 8px; padding: 14px 20px; margin-bottom: 1rem; display: flex; align-items: center; gap: 14px; }
    ...
    .kpi-card { background: linear-gradient(145deg, #071525 0%, #0a1e32 100%); border: 1px solid #0e3a5c; border-radius: 10px; padding: 12px 14px; text-align: center; box-shadow: 0 2px 12px rgba(0,100,200,0.1); height: 100%; }
    ...
    .section-title { font-size: 0.75rem; font-weight: 600; color: #29b6f6; text-transform: uppercase; letter-spacing: 0.1em; margin: 14px 0 7px 0; padding-bottom: 4px; border-bottom: 1px solid #0e3a5c; }
    ...
    .file-chip { display: inline-flex; background: rgba(79,195,247,0.1); border: 1px solid rgba(79,195,247,0.3); border-radius: 16px; padding: 2px 9px; font-size: 0.74rem; color: #4fc3f7; margin: 2px; }
    ...
    .upload-prompt { background: linear-gradient(145deg, #071525, #0a1e32); border: 2px dashed #0e3a5c; border-radius: 12px; padding: 40px 24px; text-align: center; margin-top: 1rem; }
...
with st.sidebar:
    ...
    uploaded_files = st.file_uploader("📂 上傳 GM10 CSV（可多選整月）", type=["csv"], accept_multiple_files=True)
...
for i, f in enumerate(uploaded_files):
    try:
        all_dfs.append(load_gm10_single(f.read()))
...
# 偵測時間跨度，動態決定預設抽樣
days_span_raw = (df_raw['時間'].max() - df_raw['時間'].min()).total_seconds() / 86400
if days_span_raw > 14:
    default_resample_idx = 5  # 每日
elif days_span_raw > 3:
    default_resample_idx = 3  # 15 分鐘
else:
    default_resample_idx = 1  # 5 分鐘

resample_options = ["1分鐘","5分鐘","15分鐘","30分鐘","1小時","每日平均"]
resample_map = {"1分鐘":"1min","5分鐘":"5min","15分鐘":"15min","30分鐘":"30min","1小時":"1h","每日平均":"1D"}

with st.sidebar:
    resample_opt = st.selectbox(
        f"資料抽樣（資料跨度 {days_span_raw:.1f} 天，建議 {resample_options[default_resample_idx]}）",
        resample_options,
        index=default_resample_idx
    )
...
outside_temp_keys = [s['col_key'] for s in OUTSIDE_SENSORS.values() if s['col_key'] in df_raw.columns and s.get('unit') == '°C']
...
# Date banner
date_start = df['時間'].min(); date_end = df['時間'].max()
...
with col_sl:
    time_range = st.slider("⏱️ 時間範圍", min_value=t_min, max_value=t_max,
        value=(t_min, t_max),
...
if len(dff) == 0:
    st.error("選取時間範圍內無資料，請調整滑桿。"); st.stop()
...
avg_T = np.mean(inside_vals); max_T = np.max(inside_vals); min_T = np.min(inside_vals); unif = np.std(inside_vals)
comp_T = latest.get('CH101(一號壓縮機)', np.nan)
humid  = latest.get('CH105(通道 105)',   np.nan)
amb_T  = latest.get('CH106(關鍵數據 (CH106))', np.nan)

if avg_T > alarm_high:    alarm_status = 'err'
elif avg_T > alarm_high - 2 or avg_T < alarm_low: alarm_status = 'warn'
else: alarm_status = 'ok'
...
badge_map = {'ok':'<span class="badge-ok">✅ 正常</span>','warn':'<span class="badge-warn">⚠️ 注意</span>','err':'<span class="badge-err">🚨 異常</span>'}
...
kpis = [
    ...
    ("資料筆數", f"{len(dff):,}", "筆", f"重採樣後·{resample_opt}", "#90a4ae"),
]
for kcol, (lbl, val, unit, delta, color) in zip(kpi_cols, kpis):
    kcol.markdown(f"""<div class="kpi-card">
...
with col_left:
    st.markdown("""<div style='background:linear-gradient(90deg,#071525,#0a1e32); border:1px solid #0e3a5c; border-radius:8px; padding:7px 12px; margin-bottom:8px; font-size:0.78rem; color:#5a9ab8; text-align:center;'>
    ...
    with mod_tab1:
        ...
        st.markdown(f"<div class='section-title'>Module 1 — 8 點溫度 3D 分布"
            f"<span style='font-weight:400; color:#2a6a8a; font-size:0.75rem; margin-left:10px;'>"
            f"快照：<b style='color:#4fc3f7'>{latest_time_str}</b></span></div>", unsafe_allow_html=True)

        fig_floor = go.Figure()

        # ─── 1. 半透明立方體面 (Mesh3d, alphahull=0 自動建立 6 面) ───
        fig_floor.add_trace(go.Mesh3d(
            x=[0,1,1,0, 0,1,1,0],
            y=[0,0,1,1, 0,0,1,1],
            z=[0,0,0,0, 1,1,1,1],
            alphahull=0,
        ...
        edges = [('A','B'),('A','C'),('B','D'),('C','D'),
                 ...
                 ('A','E'),('B','F'),('C','G'),('D','H')]
        # 後方+底面是「被擋住」的虛線
        hidden = {('E','G'),('G','H'),('C','G'),('D','H')}

        for p1, p2 in edges:
            x0,y0,z0 = corners[p1]; x1,y1,z1 = corners[p2]
            is_hidden = (p1,p2) in hidden or (p2,p1) in hidden
            fig_floor.add_trace(go.Scatter3d(
                x=[x0,x1], y=[y0,y1], z=[z0,z1],
                mode='lines',
                line=dict(
                    color='#1976d2' if not is_hidden else 'rgba(25,118,210,0.4)',
                    width=5 if not is_hidden else 3,
                    dash='dash' if is_hidden else 'solid'
                ),
        ...
        fig_floor.add_trace(go.Scatter3d(
            ...
            z=[0.5,  0.5,  0.5,   0.5 ],
            mode='text',
            text=['<b>前面</b>','<b>後面</b>','<b>左</b>','<b>右</b>'],
            textfont=dict(size=15, color='#ffd54f'),
            showlegend=False, hoverinfo='skip',
        ))
        ...
        if vals:
            ...
            for name, s in INSIDE_SENSORS.items():
                ...
                texts.append(f"<b>{name}</b><br>{val:.1f}°C")
                hovers.append(f"<b>{name}</b><br>{s['label']}<br>溫度: {val:.2f}°C")

            fig_floor.add_trace(go.Scatter3d(
                x=xs, y=ys, z=zs,
                mode='markers+text',
                marker=dict(size=20, color=colors_3d, opacity=0.95,
        ...
        fig_floor.update_layout(
            scene=dict(
                xaxis=dict(title='', tickvals=[0,1], ticktext=['左','右'],
                           gridcolor='rgba(14,58,92,0.3)',
                           backgroundcolor='rgba(5,13,26,0.3)',
                           tickfont=dict(color='#4a8aa8', size=11),
                           range=[-0.4, 1.4]),
                yaxis=dict(title='', tickvals=[0,1], ticktext=['前','後'],
                           gridcolor='rgba(14,58,92,0.3)',
                           backgroundcolor='rgba(5,13,26,0.3)',
                           tickfont=dict(color='#4a8aa8', size=11),
                           range=[-0.4, 1.4]),
                zaxis=dict(title='', tickvals=[0,1], ticktext=['下','上'],
                           gridcolor='rgba(14,58,92,0.3)',
                           backgroundcolor='rgba(5,13,26,0.3)',
                           tickfont=dict(color='#4a8aa8', size=11),
                           range=[-0.2, 1.3]),
        ...
        st.caption("💡 可用滑鼠拖曳旋轉、滾輪縮放。CH1-CH4 在上層，CH5-CH8 在下層")

        st.markdown('<div class="section-title">時間範圍內平均</div>', unsafe_allow_html=True)
        ...
        for row_start in range(0, len(sensor_items), 4):
            ...
            for col_idx, (name, s) in enumerate(sensor_items[row_start:row_start+4]):
                ...
                row_cols[col_idx].markdown(f"""<div class="kpi-card" style="padding:8px 10px;">
    ...
    with mod_tab2:
        ...
        for row_start in range(0, len(out_items), 3):
            ...
            for ci, (name, s) in enumerate(out_items[row_start:row_start+3]):
                if s['col_key'] not in latest.index: continue
                v = latest[s['col_key']]; unit = s['unit']
                color = "#ff8a65" if name == 'CH101' and v > 40 else ("#80deea" if unit == '%RH' else "#a5d6a7")
                row_cols[ci].markdown(f"""<div class="kpi-card">
...
with col_right:
    ...
    with tab_t:
        title_suffix = "（每日平均）" if is_daily_mode else ""
        st.markdown(f'<div class="section-title">庫內 8 點溫度時序{title_suffix}</div>', unsafe_allow_html=True)
        # 日聚合用標記+線、其他只用線
        line_mode = 'lines+markers' if is_daily_mode else 'lines'
        marker_size = 8 if is_daily_mode else 0
        ...
        if show_raw:
            for i, (name, s) in enumerate(INSIDE_SENSORS.items()):
                ...
                fig_inside.add_trace(go.Scatter(
                    ...
                    name=s['label'], mode=line_mode,
                    line=dict(width=1.8, color=INSIDE_COLORS[i]),
        ...
        fig_inside = style_fig(fig_inside, height=380)
        fig_inside.update_yaxes(title_text="溫度 (°C)")
        if is_daily_mode:
            fig_inside.update_xaxes(tickformat='%m/%d')
        st.plotly_chart(fig_inside, use_container_width=True)

    with tab_out:
        st.markdown(f'<div class="section-title">庫外溫度 & 濕度時序{title_suffix}</div>', unsafe_allow_html=True)
        fig_out = make_subplots(rows=2, cols=1, shared_xaxes=True,
                                subplot_titles=["庫外溫度 (°C)","濕度 (%RH)"], vertical_spacing=0.1)
        out_temp_sensors = ['CH101(一號壓縮機)','CH102(通道 102)','CH103(通道 103)','CH104(通道 104)','CH106(關鍵數據 (CH106))']
        ...
        for col_, lbl, color in zip(out_temp_sensors, out_temp_labels, OUT_T_COLORS):
            ...
            fig_out.add_trace(go.Scatter(
                ...
                mode=line_mode,
                line=dict(width=2, color=color), marker=dict(size=marker_size),
                hovertemplate=f"{lbl}: %{{y:.2f}}°C<extra></extra>"
            ), row=1, col=1)
        if 'CH105(通道 105)' in dff.columns:
            fig_out.add_trace(go.Scatter(
                x=dff['時間'], y=dff['CH105(通道 105)'],
                name='CH105 濕度', fill='tozeroy', mode=line_mode,
                line=dict(width=2, color='#4fc3f7'),
                marker=dict(size=marker_size),
                fillcolor='rgba(79,195,247,0.1)',
                hovertemplate="濕度: %{y:.1f}%RH<extra></extra>"
            ), row=2, col=1)
        fig_out = style_fig(fig_out, height=400)
        if is_daily_mode:
            fig_out.update_xaxes(tickformat='%m/%d', row=2, col=1)
    ...
    with tab_stat:
        if not show_stats:
            ...
            fig_stat = make_subplots(rows=2, cols=2,
                subplot_titles=["溫度均勻度 σ (°C)","Max-Min 溫差 (°C)","庫內平均溫分布","各通道平均溫"],
                vertical_spacing=0.16, horizontal_spacing=0.1)
            if 'uniformity' in dff.columns:
                fig_stat.add_trace(go.Scatter(x=dff['時間'], y=dff['uniformity'],
                    fill='tozeroy', line=dict(color='#4fc3f7', width=1.5),
                    fillcolor='rgba(79,195,247,0.1)', name='σ',
                    mode=line_mode, marker=dict(size=marker_size),
                    hovertemplate="σ: %{y:.3f}°C<extra></extra>"), row=1, col=1)
            if 'max_inside_T' in dff.columns and 'min_inside_T' in dff.columns:
                span = dff['max_inside_T'] - dff['min_inside_T']
                fig_stat.add_trace(go.Scatter(x=dff['時間'], y=span,
                    fill='tozeroy', line=dict(color='#ffa726', width=1.5),
                    fillcolor='rgba(255,167,38,0.1)', name='溫差',
                    mode=line_mode, marker=dict(size=marker_size),
                    hovertemplate="差: %{y:.2f}°C<extra></extra>"), row=1, col=2)
            if 'avg_inside_T' in dff.columns:
                fig_stat.add_trace(go.Histogram(x=dff['avg_inside_T'], nbinsx=40,
                    marker_color='#4fc3f7', opacity=0.75, name='分布'), row=2, col=1)
            ...
            fig_stat.add_trace(go.Bar(x=list(ch_avgs.keys()), y=list(ch_avgs.values()),
                marker_color=bar_colors, name='各CH平均',
                hovertemplate="%{x}: %{y:.2f}°C<extra></extra>"), row=2, col=2)
            fig_stat = style_fig(fig_stat, height=440)
...
with col_hm:
    ...
    # X 軸格式自動切換
    if is_daily_mode or days_span_raw > 7:
        x_labels = df_hm['時間'].dt.strftime('%m/%d')
    elif days_span_raw > 1:
        x_labels = df_hm['時間'].dt.strftime('%m/%d %H:%M')
    else:
        x_labels = df_hm['時間'].dt.strftime('%H:%M')
...
rename_daily = {'時間':'日期','avg_inside_T':'庫內平均(°C)','max_inside_T':'最高溫(°C)','min_inside_T':'最低溫(°C)',
                'uniformity':'均勻度σ','CH101(一號壓縮機)':'壓縮機(°C)','CH105(通道 105)':'濕度(%RH)'}
...
st.download_button("⬇️ 下載日別統計 CSV", csv_daily,
    file_name=f"GM10_daily_{date_start.strftime('%Y%m%d')}_{date_end.strftime('%Y%m%d')}.csv",
    mime="text/csv")
...
with st.expander("📋 原始資料表（區間最後 200 筆）& 下載"):
    ...
    rename_map = {'時間':'時間'}
    rename_map.update({s['col_key']: f"{n} {s['label'].split(' ',1)[1]}" for n, s in INSIDE_SENSORS.items() if s['col_key'] in dff.columns})
...
st.markdown("---")
st.markdown("""<div style='text-align:center; color:#1a4a6a; font-size:0.76rem;'>
